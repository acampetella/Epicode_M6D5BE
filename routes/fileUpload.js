...
const internalPostsImagesStorage = multer.diskStorage({
  ...
  uploads.post('/blogPosts/internal/coverUpload', internalPostsImagesUpload.single('img'), async (req, res) => {
  ...
  uploads.post('/blogPosts/cloud/coverUpload', cloudPostsImagesUpload.single('img'), async (req, res) => {
    try {
  ...
  uploads.post('/authors/internal/avatarUpload', internalAuthorsImagesUpload.single('img'), async (req, res) => {
    const url = req.protocol + '://' + req.get('host');
  ...
  uploads.post('/authors/cloud/avatarUpload', cloudAuthorsImagesUpload.single('img'), async (req, res) => {
    try {
      res.status(200).json({img: req.file.path});
    } catch (error) {
      res.status(500).send({
