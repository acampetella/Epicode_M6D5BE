...
posts.get("/blogPosts/byTitle/:title", async (req, res) => {
...
posts.get('/blogPosts/:id/comments', async (req, res) => {
  const {id} = req.params;
  try {
    const post = await BlogPostsModel.findById(id);
    if (!post) {
...
posts.get('/blogPosts/:postId/comments/:commentId', async (req, res) => {
...
posts.post('/blogPosts/:id', commentsPostsValidation, async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).send({
      errors: errors.array(),
...
posts.patch('/blogPosts/:postId/comments/:commentId', commentsPostsValidation, 
async (req, res) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return res.status(400).send({
...
posts.delete('/blogPosts/:postId/comments/:commentId', async (req, res) => {
  const {postId, commentId} = req.params;
  try {
    const post = await BlogPostsModel.findById(postId, {
      comments: {$elemMatch: {_id: commentId}}
